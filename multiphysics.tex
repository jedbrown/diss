...
Such restriction operators are usually defined geometrically, though they also have been defined algebraically for contact problems~\citep{adams2004amm}.
...
The first is to use physical arguments to define $\tilde A$ by considering the continuum operators without boundary conditions \citep[though see][]{elman2009boundary}, for example in the ``pressure convection-diffusion'' preconditioner of \citet{silvester2001efficient,kay2002pss}.
...
This method shows near mesh independence for solving the Navier-Stokes equations and while slightly less robust, it typically has better performance than the first method when Newton linearization is used~\citep{elman2008tcp}.
...
The third method is to define $\tilde A$ using sparse approximate commutators~\citep{elman2006bpb} which is a similar method to the sparse approximate inverse~\citep{grote1997parallel}.
...
For monolithic matrix formats like \code{AIJ}, \cfunc|MatGetSubMatrix| necessarily requires the matrix entries to be copied into a new data structure which as much as doubles the memory needed by an application.
...
The default implementation returns a matrix of type \code{MatSchurComplement} which applies $S$ according to its definition $D - CA^{-1}B$ by storing the four blocks and, if requested by the caller, the SIMPLE~\citep{patankar1972cph} approximation $D - C\text{diag}(A)^{-1}B$.
