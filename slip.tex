...
Although it is not used directly in our work, \eqref{eq:slip:stokes-weak} corresponds to the minimization of the viscous energy $\half D\uu\tcolon \eta\bm 1 \tcolon D\uu$ over the subspace where $\div\uu = 0$.
...
More precisely, given discrete solutions $(\uu_h,p_h) \in \VV \times \QQ$ with inf-sup constant $\beta$ and exact solution $(\uu,p)$, the velocity and pressure errors satisfy the a priori estimate
...
It is important to recognize that if the pressure space contains the piecwise constant functions, the constraint equation will force the velocity field to be exactly divergence-free when integrated over an element $e$,
\begin{gather*}
...
We use two classes of finite element space defined on a hexahedral mesh.
...
\subsubsection{Implementation of Dirichlet boundary conditions}

\subsubsection{Slip}


While use of conservative normals provides exact conservation across the interface, there is no guarantee that it will preserve realistic steady states, particularly for free surface flows where the hydrostatic contribution to pressure cannot be removed.  This issue is discussed in \cite{behr2004application}.
